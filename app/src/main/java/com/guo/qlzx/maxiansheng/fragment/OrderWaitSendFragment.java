...
import com.guo.qlzx.maxiansheng.activity.RefundActivity;
...
public class OrderWaitSendFragment extends BaseFragment implements OnItemChildClickListener, OnRVItemClickListener,
    ...
    public void getData() {
        isLazy = true;
        getOrderList(helper.getToken(), "2", page);
    }
    ...
    public void onItemChildClick(ViewGroup parent, View childView, final int position) {
        ...
        switch (btn.getText().toString()){
            ...
            case "取消订单":
                ...
                break;
            case "提醒发货":
                remind(helper.getToken(),adapter.getItem(position).getOrder_id());
                break;
            case "退货":
                deleteDialog=new AddressDeleteDialog(mContext);
                deleteDialog.setGoToUseOnclickListener(new AddressDeleteDialog.onGoToUseOnclickListener() {
                    @Override
                    public void onUseClick() {
                        returnGoods(helper.getToken(),adapter.getItem(position).getOrder_id());
                    }
                });
                deleteDialog.show();
                deleteDialog.setTitle("确认退货？");
                break;
        }
    }
     ...
     * @param order_id
     */
    private void remind(String token,String order_id){
        HttpHelp.getInstance().create(RemoteApi.class).remind(token,order_id)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new BaseSubscriber<BaseBean<Object>>(mContext, null) {
                    @Override
     ...
     * @param order_id
     */
    private void returnGoods(String token,String order_id){
        HttpHelp.getInstance().create(RemoteApi.class).returnGoods(token,order_id)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new BaseSubscriber<BaseBean<Object>>(mContext, null) {
                    @Override
                    ...
                    public void onError(Throwable throwable) {
                        super.onError(throwable);
                        ToastUtil.showToast(mContext, "接口出错！");
                    }
                });
    }
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        if (isVisibleToUser && isLazy){
            LogUtil.i("userVisible_OrderWaitSendFragment: "+isVisibleToUser);
            getData();
        }
    }
}
