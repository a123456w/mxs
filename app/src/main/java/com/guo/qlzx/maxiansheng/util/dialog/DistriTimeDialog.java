...
public class DistriTimeDialog extends Dialog {
    ...
    public DistriTimeDialog(@NonNull Context context, DistriTimeBean timeBean) {
    ...
    public void setConfimClickListener(OnConfimClickListener confimClickListener) {
    ...
    private void initData() {
        ...
        String time = TimeUtil.getSimpleDate(timeBean.getNow_time());
        String nowTime = time+ " " + timeBean.getBegin_time();
        try {
            if (format.parse(nowTime.toString()).getTime() + (timeBean.getTimes()*1000)
                    < calendar.getTimeInMillis()) {
                days.add(new DistriDayBean("今天", true));
            }
            days.add(new DistriDayBean("明天", days.size()>0?false:true));
        ...
        try {
            ...
            if (day == 0) {
                for (int i = 0; i < num; i++) {
                    ...
                    if (tempstartTime > Long.valueOf(timeBean.getNow_time()) * 1000) {
